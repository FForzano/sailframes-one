...
**GPIO assignments verified against E1 firmware source `sailframes_edge.ino`:**
...
| SCK | VSPI CLK | GPIO18 |
| LED | Backlight | **GPIO25** (via current-limiting resistor, ~22 Ω) — matches E1 firmware (`BL=25` in `sailframes_edge.ino`). GPIO19 was a stale header-comment in E1's source and is reassigned in B1 to Hall-latch state readback. |
...
- ✓ **GPIO assignments verified against E1 firmware source `sailframes_edge.ino` (2026-05-17):**
